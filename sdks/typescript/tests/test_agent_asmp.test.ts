/**
 * Agent integration tests: OpenAI (mini) drives ASMP via client and registry.
...
function registryAgentTools(registry: ASMPClientRegistry, runIds: Record<string, string>, options: { addServer?: boolean } = {}) {
  const tools: Array<{ type: "function"; function: { name: string; description: string; parameters: { type: "object"; properties: Record<string, unknown>; required: string[] } } }> = [
    {
      ...
      function: {
        name: "list_servers",
        description: "List available ASMP servers and embedded FSMs (id and type).",
  ...
  if (options.addServer) {
    tools.push({
      ...
      function: {
        name: "add_server",
        description: "Dynamically add an ASMP server by URL (e.g. from a skill or prompt).",
        parameters: {
          type: "object",
          properties: {
            id: { type: "string", description: "Label for this server" },
            base_url: { type: "string", description: "Base URL of the ASMP server" },
          },
...
describe("Agent drives workflow via OpenAI mini", () => {
  it.skipIf(!OPENAI_API_KEY)(
    ...
    async () => {
      ...
      const client = new ASMPClient(baseUrl, 10_000);
...
describe("Agent with registry: HTTP server + embedded FSM in parallel", () => {
  it.skipIf(!OPENAI_API_KEY)(
    ...
    async () => {
      const wfEmbedded = new ASMPWorkflow("embedded-wf", "INIT", transitions, "memory:")
      ...
      const registry = new ASMPClientRegistry({
        ...
        localFsms: { embedded: new LocalASMPBackend(wfEmbedded, {}) },
      });
  ...
  it.skipIf(!OPENAI_API_KEY)(
    ...
    async () => {
      ...
      const messages: Message[] = [
        {
          role: "user",
          content: `A new ASMP server is available at ${dynamicUrl}. First call add_server with id "dynamic" and base_url "${dynamicUrl}". Then call list_servers, start a run on "dynamic" with start_run, and complete it by calling transition with action "start". Reply DONE when the workflow is complete.`,
        },
      ];
      const maxCalls = 15;
      for (let i = 0; i < maxCalls; i++) {
