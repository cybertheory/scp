# SWP Stage Integrations (Tools & Resources)

Stage integrations let you attach **tools** (callable) and **resources** (read-only) to specific FSM states. Unlike MCP’s global tool menu, SWP exposes only the integrations that are valid in the **current state**, so the agent gets a minimal, FSM-gated surface.
...
## 5. Relation to other SWP features
