#!/usr/bin/env python3
"""Check that sdks/python/scp/openapi.json is in sync with spec/openapi.json.
...
PYTHON_COPY = ROOT / "sdks" / "python" / "scp" / "openapi.json"
...
def main() -> int:
    ...
    if normalize(spec) != normalize(python_spec):
        print("OpenAPI out of sync: spec/openapi.json != sdks/python/scp/openapi.json", file=sys.stderr)
        print("Update spec/openapi.json first, then copy to sdks/python/scp/openapi.json", file=sys.stderr)
        return 1
    return 0
