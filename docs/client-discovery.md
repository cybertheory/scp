...
```typescript
const registry = new SWPClientRegistry({});  // or existing config
...
```typescript
const configFromFile = fs.readFileSync("swp.json", "utf-8");
const registry = new SWPClientRegistry({ config: configFromFile });
// Or merge later:
registry.addConfig(parseSWPClientConfig(anotherJsonString));
...
```typescript
registry.addLocalFsm("helper", new LocalSWPBackend(helperWorkflow, {}));
```

### Parse config only

```typescript
import { parseSWPClientConfig } from "swp-sdk";

const config = parseSWPClientConfig(fs.readFileSync("swp.json", "utf-8"));
